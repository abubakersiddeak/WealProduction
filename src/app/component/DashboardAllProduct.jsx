...
const DashboardAllProduct = ({ product, handleUpdate, handleDelete }) => {
  ...
  useEffect(() => {
    if (product) {
      setFormData({
        ...
        buyPrice: product.buyPrice !== null && product.buyPrice !== undefined ? String(product.buyPrice) : "",
        categoryGender: product.category?.gender || "",
        categoryType: product.category?.type || "",
        categoryScollection: product.category?.scollection || "",
        sizes: Array.isArray(product.sizes) ? product.sizes : [], // Simplified
  ...
  const totalProductQuantity = product.sizes.reduce(
    (sum, s) => sum + s.quantity,
    0
  );

  // Determine stock status based on total quantity
  const isProductInStock = totalProductQuantity > 0;
  console.log("Product prop:", product); // Add this line
  return (
    <>
      {/* Product Card */}
      <div className="bg-cyan-200  rounded-xl shadow-xl hover:shadow-purple-500/30 transition-all duration-300 p-6 cursor-pointer transform hover:-translate-y-1">
        ...
        <p className="text-sm mb-1 flex items-center">
          ...
          {isProductInStock ? ( // Use calculated stock
        ...
        <p className="text-sm  mb-1">
          <strong className="">Total Quantity:</strong> {totalProductQuantity}
        </p>
        <p className="text-sm  mb-1 flex items-center">
                ...
                <div>
                  ...
                  {formData.sizes.map((sizeObj, index) => (
                    <div key={index} className="flex items-center gap-4 mb-2">
                      <input
                        type="text"
                        placeholder="Size (e.g., S, M, Regular Size)"
                        className="w-24 px-3 py-2 bg-gray-800 border border-gray-600 rounded-md text-gray-100 focus:outline-none focus:ring-2 focus:ring-purple-500"
                        value={sizeObj.size}
                        onChange={(e) => {
