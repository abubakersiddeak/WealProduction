...
export default function Product({ product }) {
  ...
  const [instock, setInstock] = useState(true); // Revert to initial true, will be updated by useEffect
  ...
  const [selectedSize, setSelectedSize] = useState(
    sizes.length > 0 ? sizes[0] : null // Initialize with the first size object if available, otherwise null
  );
  ...
  useEffect(() => {
    ...
    // If sizes array is available and no size is selected, select the first available size
    if (sizes.length > 0 && !selectedSize) {
      setSelectedSize(sizes.find((s) => s.quantity > 0) || sizes[0]); // Select first available, or just first
    }
  }, [sizes, selectedSize]); // Add sizes to dependency array
            ...
            {sizes.length > 0 && (
              <div>
                ...
                >
                  {sizes.map((size, i) => (
                    <option
                      key={size.size} // Use size.size as key
                      value={size.size}
                        ...
                        addToCart(
                          ...
                          relProduct.sizes && relProduct.sizes.length > 0
                            ? relProduct.sizes.find((s) => s.quantity > 0) ||
                                relProduct.sizes[0] // Pass the first available size object, or just the first
                            : { size: "Regular Size", quantity: 0 } // Fallback to an object for consistency
                        );
