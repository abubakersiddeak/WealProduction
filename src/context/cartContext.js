...
export const CartProvider = ({ children }) => {
  ...
  const addToCart = (product, quantity, selectedSize) => {
    const existingItemIndex = cartItems.findIndex(
      (item) => item.product._id === product._id && item.size.size === selectedSize.size
    );
  ...
  const removeFromCart = (product, size) => {
    setCartItems((currentCartItems) => {
      const updatedCart = currentCartItems.filter(
        (item) => !(item.product._id === product._id && item.size.size === size.size)
      );
      return updatedCart;
    });
  };
  ...
  const updateQuantity = (product, size, newQuantity) => {
    setCartItems((currentCartItems) => {
      return currentCartItems.map((item) => {
        if (item.product._id === product._id && item.size.size === size.size) {
  ...
  const clearCheckedOutItems = (checkedOutItems) => {
    setCartItems((currentCartItems) => {
      const checkedOutItemIds = new Set(
        checkedOutItems.map((item) => `${item.product._id}-${item.size.size}`)
      );

      const updatedCart = currentCartItems.filter((item) => {
        return !checkedOutItemIds.has(`${item.product._id}-${item.size.size}`);
      });
